...
const FLAG_ANCHOR: [number, number, number] = [2.9, 2.2, -1.8];
const ROD_LEN = 0.3;
...
export default function Clutter({ position = [0, 0, 0], rotationY = 0 }: PropProps) {
  ...
  return (
    <group position={position} rotation={[0, rotationY, 0]}>
      ...
      {/* (b) Potted palm */}
      <group position={PLANT_BASE}>
        <mesh material={flatMat('woodRed')} castShadow receiveShadow>
          <latheGeometry args={[potPoints, 8]} />
        </mesh>
        <mesh
          position={[0, POT_RIM_Y + (STEM_TOP_Y - POT_RIM_Y) / 2, 0]}
          material={flatMat('plantDark')}
          castShadow
        >
          <cylinderGeometry args={[0.015, 0.02, STEM_TOP_Y - POT_RIM_Y, 6]} />
        </mesh>
        {FROND_CONFIG.map((frond, i) => {
          const azimuth = (i * (Math.PI * 2)) / FROND_COUNT;
          return (
            <group key={i} position={[0, STEM_TOP_Y, 0]} rotation={[0, azimuth, 0]}>
              <group rotation={[0, 0, -frond.pitch]}>
                <mesh position={[FROND_LEN / 2, 0, 0]} material={flatMat(frond.mat)} castShadow>
                  <boxGeometry args={[FROND_LEN, FROND_T, FROND_W]} />
                </mesh>
              </group>
            </group>
          );
      ...
      {/* (d) Scattered desk papers */}
      {PAPER_CONFIG.map((p, i) => (
        <mesh key={i} position={p.pos} rotation={[0, p.rotY, 0]} material={flatMat('paper')}>
          <boxGeometry args={[PAPER_W, PAPER_T, PAPER_D]} />
        </mesh>
      ))}

      {/* (e) Wooden crate + bottle */}
      <mesh
      ...
      <mesh position={BOTTLE_POS} material={flatMat('plantDark')} castShadow>
        <latheGeometry args={[bottlePoints, 8]} />
      </mesh>
    </group>
  );
}
